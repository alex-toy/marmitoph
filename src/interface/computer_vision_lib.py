...
def plot_dataset_distribution (stats, num_cols=5, width=10, height=5, histogram_bins = 10, histogram_range=[0, 1000], figure_padding=4):
  ...
  if (num_rows==1 or num_cols==1):
    ...
    for index, food_brand in enumerate(class_count_dict):
        plt.text(food_brand, class_count_dict[food_brand]+1, str(class_count_dict[food_brand]))
...
def get_food_classes_dict(testY,testLabels):

  number_of_classes=np.unique(testLabels).size
  food_dict={}

  for i,label in enumerate(testY):
      food_dict[np.argmax(label)]=testLabels[i]
      #print(len(food_dict))
      length=len(food_dict)
      if (length==number_of_classes):
          break
  #print(sorted(food_dict.items()))
  #print(food_dict)
  return food_dict,number_of_classes



def plot_test_image(testX, image_index, predictions_array, true_binary_labels, food_dict=None):
    """
    ...
    if (food_dict==None):
      ...
      ##print(food_dict[predicted_binary_label])
       plt.xlabel("{} predicted: {} {:2.0f}% (true: {})".format(image_index, food_dict[predicted_binary_label],
                                  100*np.max(single_predictions_array),
                                  food_dict[true_binary_label]),
                                  color=color)
...
def plot_food_sample_predictions(testX, predictions_array, testY,number_of_classes=3,num_rows = 10, num_cols = 4, width=None, height=None, is_random=True):
    """
        this method plots a sample of predictions from the test dataset and highlight correct and wrong predictions
        testX: this is the test dataset
        image_index: index of the image that we will plot from the test dataset
...
def plot_food_sample_predictions_v2(testX, testY, testLabels, predictions_array, num_rows = 10, num_cols = 4, width=None, height=None, is_random=True, plot_bars=True):
    """
        this method plots a sample of predictions from the test dataset and highlight correct and wrong predictions
        testX: this is the test dataset
        image_index: index of the image that we will plot from the test dataset
    ...
    food_dict,number_of_classes=get_food_classes_dict(testY,testLabels)
    #print(food_dict)
    ...
    for i in range(num_images):
        ...
        #if (plot_bars==True):
        plt.subplot(num_rows, 2*num_cols, 2*i+1)
        plot_test_image(testX, image_index, predictions_array[image_index], true_binary_labels, food_dict)
    #---------------
        plt.subplot(num_rows, 2*num_cols, 2*i+2)
        plot_value_array(image_index, predictions_array[image_index], true_binary_labels, number_of_classes)
...
def plot_misclassifications(testX, testY, testLabels, predictions_array, num_rows = 10, num_cols = 4, width=None, height=None, is_random=True, plot_bars=True):
    ...
    for i in range(num_images):
        ...
        #---------------
            ...
            plot_value_array(image_index, predictions_array[image_index], true_binary_labels, number_of_classes)
            count=count+1
            misclassified_indices[image_index]=[food_dict[true_binary_labels[image_index]],food_dict[predicted_binary_label]]
